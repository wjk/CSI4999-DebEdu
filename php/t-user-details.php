...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("UPDATE TEACHER_USER SET REAL_NAME = ? WHERE USER_NAME = ?;");
    $stmt->bind_param("ss", $new_full_name, $user_name);
    $stmt->execute();

    $new_email = $_POST["email"];
    $stmt = $conn->prepare("UPDATE TEACHER_USER SET EMAIL = ? WHERE USER_NAME = ?;");
...
function get_full_name($user_name, $conn) {
    $stmt = $conn->prepare("SELECT REAL_NAME FROM TEACHER_USER WHERE USER_NAME = ?;");
...
function get_email($user_name, $conn) {
    $stmt = $conn->prepare("SELECT EMAIL FROM TEACHER_USER WHERE USER_NAME = ?;");
    $stmt->bind_param("s", $user_name);
...
<body>
    <div class="main-container">
        ...
        <h1>Details for <?php echo($user_name) ?></h1>
            ...
            <?php } ?>
            <p>
                You are a <span class="bold">teacher</span>.
            </p>
        </p>
        <form id="user-form" method="POST" action="t-user-details.php">
